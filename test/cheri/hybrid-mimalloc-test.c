...
static int SCALE   = 1;      // scaling factor
...
#define custom_calloc(n,s)    slimguard_hc_calloc(n,s)
#define custom_realloc(p,s)   slimguard_hc_realloc(p,s)
#define custom_free(p)        slimguard_hc_free(p)
#else
...
static void * __capability atomic_exchange_ptr(volatile void * __capability * __capability p, void* __capability newval);
...
static void * __capability alloc_items(size_t items, random_t r) {
  if (chance(1, r)) {
  ...
  if (items==0) items = 1;
  uintptr_t * __capability p = custom_calloc(items,sizeof(uintptr_t));
  if (p != NULL) {
...
static void stress(intptr_t tid) {
  ...
  void * __capability * __capability data = NULL;
  size_t data_size = 0;
  size_t data_top = 0;
  void * __capability * __capability retained = (void * __capability * __capability)custom_calloc(retain,sizeof(void * __capability));
  size_t retain_top = 0;
  ...
  while (allocs > 0 || retain > 0) {
    if (retain == 0 || (chance(50, &r) && allocs > 0)) {
      ...
      if (data_top >= data_size) {
        data_size += 100000;
        data = (void * __capability * __capability)custom_realloc(data, data_size * sizeof(void * __capability));
      }
      data[data_top++] = alloc_items(1ULL << (pick(&r) % max_item_shift), &r);
    }
    else {
      // 25% retain
      void * __capability tmp = alloc_items( 1ULL << (pick(&r) % max_item_retained_shift), &r);
      retained[retain_top++] = tmp;
      retain--;
    }
    ...
    if (chance(25, &r) && data_top > 0) {
      ...
      void * __capability p = data[data_idx];
      void * __capability q = atomic_exchange_ptr(&transfer[transfer_idx], p);
      data[data_idx] = q;
    }
  }
  ...
  for (int n = 0; n < ITER; n++) {
    ...
    for (int i = 0; i < TRANSFERS; i++) {
      if (chance(50, &r) || n + 1 == ITER) { // free all on last run, otherwise free half of the transfers
        void * __capability p = atomic_exchange_ptr(&transfer[i], NULL);
        free_items(p);
      }
    }
    // mi_collect(false);
...
static void run_os_threads(size_t nthreads, void (*fun)(intptr_t)) {
  thread_entry_fun = fun;
  pthread_t * __capability threads = custom_calloc(nthreads,sizeof(pthread_t));
  memset_c(threads, 0, sizeof(pthread_t) * nthreads);
  //pthread_setconcurrency(nthreads);
  for (uintptr_t i = 0; i < nthreads; i++) {
    pthread_create((__cheri_fromcap pthread_t *)(&threads[i]), NULL, &thread_entry, (void*)i);
  }
  for (size_t i = 0; i < nthreads; i++) {
...
#include <stdatomic.h>
static void * __capability atomic_exchange_ptr(volatile void * __capability * __capability p, void* __capability newval) {
  void **tmp = (__cheri_fromcap void **)p;
  return (__cheri_tocap void * __capability)atomic_exchange((volatile _Atomic(void*)*)tmp, (__cheri_fromcap void *)newval);
}
#endif
