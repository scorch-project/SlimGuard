...
void
#if __STDC__
mem_init(unsigned char * __capability ptr, unsigned long size)
#else
...
#endif
{
        ...
        if(size > sizeof(unsigned long)) {
                ...
                unsigned long *decap_ptr = (unsigned long *)(__cheri_addr unsigned long)ptr;
                *decap_ptr = (__cheri_addr unsigned long)ptr ^ size;
                i = TEST_INC;
        } else
                i = 0;
        for(; i<size; i+=TEST_INC) {
                j = (__cheri_addr unsigned long)ptr ^ i;
                ptr[i] = ((j ^ (j>>8)) & 0xFF);
        }
        j = (__cheri_addr unsigned long)ptr ^ (size-1);
        ptr[size-1] = ((j ^ (j>>8)) & 0xFF);
}

int
#if __STDC__
mem_check(unsigned char * __capability ptr, unsigned long size)
#else
...
#endif
{
        ...
        if(size > sizeof(unsigned long)) {
                ...
                unsigned long *decap_ptr = (unsigned long *)(__cheri_addr unsigned long)ptr;
                if(*decap_ptr != ((__cheri_addr intptr_t)ptr ^ size)) {
				  printf ("failed size check: expected %lx, found %lx!\n",
						  ((__cheri_addr intptr_t)ptr ^ size), *decap_ptr);
				  return 1;
				}
                i = TEST_INC;
        } else
...
#endif
{
        ...
        if(r < PROB_MEMALIGN) {
#if !defined(_WIN32)
                if(m->size > 0) slimguard_hc_free(m->ptr);
                m->size = random_size(size);
#if PROB_MEMALIGN
                m->ptr = (unsigned char *)slimguard_hc_memalign(4 << RANDOM(8), m->size);
#endif
                ...
                if(m->size == 0) {
                        ...
                        m->ptr = (unsigned char *)slimguard_hc_malloc(1);
#endif
                }
#if TEST > 2
                key = RANDOM(256);
                ...
                m->size = random_size(size);
                /*printf("realloc %d\n", (int)m->size);*/
                m->ptr = slimguard_hc_realloc(m->ptr, m->size);
#if TEST > 2
                ...
                if(m->size > 0) slimguard_hc_free(m->ptr);
                m->size = random_size(size);
                m->ptr = slimguard_hc_calloc(m->size, 1);
#if TEST > 2
                ...
                if(m->size > 0) slimguard_hc_free(m->ptr);
                m->size = random_size(size);
                m->ptr = slimguard_hc_malloc(m->size);
        }
        if(!m->ptr) {
...
bin_test()
{
        ...
        for(b=0; b<bins; b++) {
                if((v = mem_check(m[b].ptr, m[b].size))) {
                        printf("bin_test: memory corrupt! m[%d].ptr = 0x%lx, m[%d].size = %lu, ",
							   b, (__cheri_addr intptr_t)m[b].ptr, b, m[b].size);
						printf ("error = %d\n", v);
                        exit(1);
                }
        }
        for(b=0; b<sbins; b++) {
                if(mem_check(sm[b].ptr, sm[b].size)) {
                        printf("bin_test: memory corrupt! sm[%d].ptr = 0x%lx, sm[%d].size = %lu\n",
							   b, (__cheri_addr intptr_t)sm[b].ptr, b, sm[b].size);
                        exit(1);
                }
        }
}
...
#endif
{
        ...
        int argc = 3;
        const char * const argv[] = {"hello", "1000", "1048576" };
...
//                        sum = (long)sbrk(0);
        ...
        /* Correct sbrk values. */
        sbrk_max -= (__cheri_addr long)base_ptr;
        sbrk_used_sum -= (double)count*(__cheri_addr long)base_ptr;
#ifdef VERBOSE
